...
export default function AuthScreen({ onLoggedIn }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post(`/auth/listener/${mode}`, { name, phone, password });
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <View style={styles.logoWrap}>
          ...
          <Text style={styles.logo}>Humsafar Team</Text>
          <Text style={styles.tagline}>Kisi ki baat sun kar uska din behtar banayein</Text>
        </View>

        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {mode === 'login' ? 'Team login 🎧' : 'Naya listener account'}
          </Text>
          ...
          <TouchableOpacity style={styles.button} onPress={submit} disabled={loading} activeOpacity={0.85}>
            {loading ? <ActivityIndicator color="#fff" /> : (
              <Text style={styles.buttonText}>
                {mode === 'login' ? 'Login karein' : 'Register karein'}
              </Text>
            )}
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => setMode(mode === 'login' ? 'register' : 'login')}>
            <Text style={styles.switchText}>
              {mode === 'login' ? 'Naya listener account banayein' : 'Pehle se account hai? Login'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  logo: { fontSize: 30, fontWeight: '800', color: C.text, letterSpacing: 0.5 },
  tagline: { color: C.muted, marginTop: 6, fontSize: 13, textAlign: 'center' },
  card: { backgroundColor: C.surface, borderRadius: radius.lg, padding: 20, ...shadow },
